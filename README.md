...
```text
src/
├── dualSimplex.js            ← математичне ядро на Fraction.js (ESM export)
├── gomory.js                 ← обгортка Гоморі (без похибок заокруглення)
├── DualSimplexApp.jsx        ← оркестратор
├── MatrixInput.jsx           ← форма + Gomory checkbox
├── StepVisualizer.jsx        ← покроковий рендер + логіка PDF експорту
├── PrintReport.jsx           ← шаблон для генерації багатосторінкового PDF-звіту
└── SimplexApp.module.scss    ← всі стилі SCSS Modules
```

Ключові моменти реалізації:
- **Математична точність:** Ядро повністю переписане з використанням `fraction.js`. Це усуває проблему чисел з рухомою комою (IEEE 754) та забезпечує ідеально точний розрахунок відсікань Гоморі без нескінченних дробів.
- **Експорт у PDF:** Застосунок генерує повноцінний академічний звіт формату А4 за допомогою `html2pdf.js`.
- **Маркування змінних:** Мітки змінних (`varLabels`) будуються від першого кроку — тому x₁, x₂, s₁, s₂ стабільні, а нові слабкі змінні від відсікань автоматично отримують назви s₃, s₄... Чіп розв'язку показує лише оригінальні змінні та має фіолетовий варіант зі значком ∈ ℤ для цілочислового результату.
...
## 2. Встановлення залежностей, SCSS та пакету для деплою

```bash
npm install fraction.js html2pdf.js
